...
func parseSearchRequest(pkt *Packet) (*SearchRequest, error) {
	if len(pkt.Items) != 8 {
		return nil, &ProtocolError{Reason: "search request should have 8 items"}
	}
	var ok bool
	req := &SearchRequest{}
	if req.BaseDN, ok = pkt.Items[0].Str(); !ok {
		return nil, &ProtocolError{Reason: "can't parse baseObject for search request"}
	}
	scope, ok := pkt.Items[1].Int()
	if !ok {
		return nil, &ProtocolError{Reason: "can't parse scope for search request"}
	}
	req.Scope = Scope(scope)
	deref, ok := pkt.Items[2].Int()
	if !ok {
		return nil, &ProtocolError{Reason: "can't parse derefAliases for search request"}
	}
	...
	for _, it := range pkt.Items[7].Items {
		s, ok := it.Str()
		if !ok {
			return nil, &ProtocolError{Reason: "can't parse attribute from list for search request"}
		}
...
func parseSearchResultResponse(pkt *Packet) (*SearchResult, error) {
	if len(pkt.Items) != 2 {
		return nil, &ProtocolError{Reason: "search result response should have 2 items"}
	}
	var ok bool
	res := &SearchResult{}
	res.DN, ok = pkt.Items[0].Str()
	if !ok {
		return nil, &ProtocolError{Reason: "failed to parse dn for search result response"}
	}
	res.Attributes = make(map[string][][]byte)
	for _, p := range pkt.Items[1].Items {
		if len(p.Items) != 2 {
			return nil, &ProtocolError{Reason: "search result response attribute should have 2 items"}
		}
		name, ok := p.Items[0].Str()
		if !ok {
			return nil, &ProtocolError{Reason: "failed to parse attribute name in search result response"}
		}
		for _, p2 := range p.Items[1].Items {
			value, ok := p2.Bytes()
			if !ok {
				return nil, &ProtocolError{Reason: "failed to parse attribute value in search result response"}
			}
