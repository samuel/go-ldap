...
func parseModifyDNRequest(pkt *Packet) (*ModifyDNRequest, error) {
	if len(pkt.Items) < 3 || len(pkt.Items) > 4 {
		return nil, &ProtocolError{Reason: "wrong number of items"}
	}
	var ok bool
	req := &ModifyDNRequest{}
	...
	if !ok {
		return nil, &ProtocolError{Reason: "invalid newrdn"}
	}
	req.DeleteOldRDN, ok = pkt.Items[2].Bool()
	if !ok {
		return nil, &ProtocolError{Reason: "invalid deleteoldrdn"}
	}
	if len(pkt.Items) == 4 {
		req.NewSuperior, ok = pkt.Items[3].Str()
		if !ok {
			return nil, &ProtocolError{Reason: "invalid newSuperior"}
		}
	}
	return req, nil
}
